...
export default class SuratKeluar extends Component {
  constructor(props) {
    super(props);
      this.state = {
        tableHead: ['Tanggal', 'Perihal', 'Kepada', 'Aksi'],
        tableData: []
      }
    }

    componentDidMount(){
      return fetch('http://demo7784458.mockable.io/SuratKeluar')
      .then((response) => response.json())
      .then((responseJson) => {
        let suratKeluar = responseJson.SuratKeluar
        var arrayData = []
        for(let i = 0;i < suratKeluar.length; i++){
          let arrayItem = []
          arrayItem.push(suratKeluar[i].tanggal)
          arrayItem.push(suratKeluar[i].perihal)
          arrayItem.push(suratKeluar[i].kepada)
          arrayItem.push('')
  ...
  _alertIndex(index) {
    Alert.alert(`Menampilkan data SuratKeluar baris ke ${index + 1}`);
  }
  render() {
    ...
    const element = (data, index) => (
      <TouchableOpacity>
        <View style={styles.btn1}>
          <Text style={styles.btnText} onPress = {() => navigate('DetailSuratKeluar')}>Lihat</Text>
        </View>
      </TouchableOpacity>
